...
This is a very broad definition which essentially defines requirements for how easy can the system be modified for change.
...
A substitute that this paper explores is replacing missing semantic data with a heuristic based approach where file risk get identified through metrics. This necessitates shifting the focus from why a file changed to how much it changed and what it changed with. Setting the focus for the heuristics to be temporal while ignoring the structural. Using the scale of the data to supplement the natural loss of fidelity that this created. Naturally leading to the study of Code Churn.
...
Current state of the art tooling such as Codescene has made strides in moving towards behavioural code analysis. Codescene"s creation of "refactoring targets", a system in which traditional static analysis is used to identify high risk files is overlaid a heatmap of developer activity to focus in on problematic active files. Addressing a fundamental issue of traditional static analysis: the "urgency" gap. This approach is grounded in the principle that tech debt is only an immediate risk if it exits in "active code". These "refactoring targets" provide a pragmatic prioritisation of maintenance effort allowing developers to focus on the 1-3% of the codebase that typically accounts for the majority of maintenance overhead. 
...
=== Data collection
 custom utility tool in rust was created to aggregate metadata from JOSS. This tool interfaced with Github API to gather repository-level metrics(contributor count, collaborator count). The resulting data was analysed by a python pipeline with pandas @team_pandas-devpandas_2020 to determine the final dataset. 
...
This is a bespoke formula normalising per file metrics proportionally to repository wide metrics to allow accurate comparison across repositories. The core justification to use this formula is derived from three core principles:
...
=== Dataset Summary
The final balanced dataset consists of 4,000 observations from 212 research repositories well distributed across each repository //FIGURE HERE
. This 50/50 class distribution prevents the classifier from developing a majority-class bias, ensuring that the resulting decision rules are equally sensitive to the characteristics of "Hub" files.
...
The extraction on human readable rules is a strong step towards practical application. However they cannot be trusted without empirical testing as without it they are worse than expert rules. Some examples can be seen here //REFERENCE THE TWO DIAGRAMS
...
To evaluate the performance and generalizability of the proposed models, a multi-stage validation framework was implemented.
