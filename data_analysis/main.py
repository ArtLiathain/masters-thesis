...
print(f"Projects dropped as outliers (> {OUTLIER_MAX_COMMITS} commits): {
      len(df) - len(df_no_outliers)}")
print(f"Projects filtered out (below {MIN_COMMITS} commits or {
      MIN_CONTRIBUTORS} contributors): {len(df_no_outliers) - len(df_final)}")
...
sns.histplot(df_final['contributor_count'], kde=False, ax=axes[0, 1], color='#e74c3c',
...
sns.scatterplot(data=df_final, x='contributor_count', y='commit_count', ax=axes[1, 0],
...
axes[1, 1].set_title('D. Distribution of Repository Size (KB)',
                     fontsize=15, fontweight='bold')
